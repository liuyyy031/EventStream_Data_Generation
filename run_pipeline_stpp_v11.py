#!/usr/bin/env python3
"""Parallel batch driver for grounded conditional-envelope STPP v11."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v11 import demo_stpp_generation_v11
    from stpp_adapter_v11 import STPPV11Config
    ...
    if not quality["passed"]:
        ...
        raise RuntimeError(f"v11 quality gates did not pass: {failed}")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v11.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v11"),
    )
