#!/usr/bin/env python3
"""Parallel batch driver for explicit event-peak STPP v8."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v8 import demo_stpp_generation_v8
    from stpp_adapter_v8 import STPPV8Config
    ...
    if not quality["passed"]:
        ...
        raise RuntimeError(f"v8 quality gates did not pass: {failed}")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v8.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v8"),
    )
