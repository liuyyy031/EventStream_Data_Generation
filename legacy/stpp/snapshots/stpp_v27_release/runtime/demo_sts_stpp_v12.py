#!/usr/bin/env python3
"""STPP v12 entry point with explicit temporal contracts and guarded Judge QA."""
...
from safe_judge_generator_v12 import TemporalContractJudgeNetworkSTPPGenerator
...
def _judge_contract_consistent(judge_audit: Dict[str, Any]) -> bool:
    if not judge_audit.get("required"):
        return True
    judgment = judge_audit.get("last_judgment") or {}
    if judge_audit.get("approved") is True:
        return bool(
            judgment.get("approved") is True
            and judgment.get("error_source") is None
            and not judgment.get("issues")
            and judgment.get("judge_contract_consistent") is True
        )
    return bool(
        judgment.get("approved") is False
        and judgment.get("error_source") in {"agent1", "agent2"}
        and bool(judgment.get("issues"))
        and judgment.get("judge_contract_consistent") is True
    )


def _agent1_feedback_v12(judgment: Dict[str, Any]) -> str:
    findings = []
    ...
    return (
        ...
        "Obey the STPP v12 time-axis, non-wrapping-window, explicit-days, "
        "single-self-pattern, and cumulative-lag contracts. Return only the "
        "revised scenario.\n\n"
        + "\n".join(findings)
    )


def parse_scenario_with_judge_audit_v12(
    generator: TemporalContractJudgeNetworkSTPPGenerator,
    scenario: str,
    ...
    print("\n=== STPP v12 temporal-contract Agent/Judge validation loop ===")
    while True:
        ...
        for agent2_attempt in range(1, max_agent2_attempts + 1):
            ...
            if feedback_history:
                cumulative_feedback = (
                    "Fix every still-applicable parsing mismatch without "
        ...
        if (
            ...
            current_scenario = generator.generate_scenario_description(
                ...
                previous_feedback=_agent1_feedback_v12(last_judgment),
...
def _write_outputs_v12(
    ...
    prefix = f"{domain_clean}_node{ts_data.shape[0]}_stpp_v12_{timestamp}"
    complete_data = {
        "timestamp": timestamp,
        "generator_family": "explicit_temporal_contract_graph_stpp_v12",
    ...
    temporal = generation_info["temporal_contract_audit"]
    with description_path.open("w", encoding="utf-8") as handle:
        handle.write("=== STReasoner Explicit Temporal-Contract STPP v12 ===\n\n")
        ...
        handle.write(f"Time-axis mode: {temporal.get('mode')}\n")
        handle.write(f"Repeat period: {temporal.get('repeat_period')}\n")
        handle.write(f"Temporal repairs: {temporal.get('num_changes', 0)}\n")
        handle.write(
        ...
        handle.write("Temporal contract audit\n-----------------------\n")
        handle.write(json.dumps(temporal, indent=2, ensure_ascii=False))
        ...
        handle.write(
            json.dumps(
                generation_info["semantic_peak_audit"],
                indent=2,
                ensure_ascii=False,
            )
        )
        ...
        handle.write(
            json.dumps(
                generation_info["propagation_cycle_audit"],
                indent=2,
                ensure_ascii=False,
            )
        )
...
def demo_stpp_generation_v12(
    enabled_judges: List[int] | None = None,
    enable_logging: bool = True,
    num_nodes: int = 3,
    domain: str = "traffic",
    generate_viz: bool = False,
    stpp_config: STPPV12Config | None = None,
    output_dir: str = "output_stpp_v12",
) -> Dict[str, Any]:
    enabled_judges = [1] if enabled_judges is None else enabled_judges
    config = stpp_config or STPPV12Config()
    if generate_viz:
        print("v12 does not generate the legacy SDE visualization.")
    ...
    if 1 in enabled_judges:
        scenario, parsed, judge_audit, agent2_audit = (
            parse_scenario_with_judge_audit_v12(generator, scenario)
        )
    else:
    ...
    generator.seq_len = seq_len
    structured_scenario, temporal_audit = canonicalise_temporal_contract_v12(
        parsed, seq_len
    )
    ...
    ts_data, events, generation_info = simulate_stpp_for_streasoner_v12(
...
def main() -> int:
    ...
    parser.add_argument("--output_dir", default="output_stpp_v12")
    parser.add_argument(
        "--aggregation",
    ...
    result = demo_stpp_generation_v12(
