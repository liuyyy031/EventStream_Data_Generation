...
def _declared_route_edge_keys(
    structured_scenario: Mapping[str, Any],
) -> set[str]:
    """Return edges that an enforced event route can actually traverse."""
    ...
    for pattern in patterns:
        try:
            path = [int(node) for node in (pattern.get("path", []) or [])]
...
def _lag_report(
    ...
    all_configured_edges = {
        f'{int(edge["source"])}->{int(edge["target"])}': max(
            float(edge.get("time_lag", 1)), 0.01
        )
        for edge in structured_scenario.get("edges", [])
    }
    declared_route_edges = _declared_route_edge_keys(structured_scenario)
    routes_are_enforced = bool(
        getattr(config, "enforce_declared_event_routes", False)
    )
    if routes_are_enforced and declared_route_edges:
        configured_edges = {
            key: value
            for key, value in all_configured_edges.items()
            if key in declared_route_edges
        }
        evaluation_scope = "declared_event_route_edges"
    else:
        configured_edges = all_configured_edges
        evaluation_scope = "all_configured_edges"
    ignored_configured_edges = sorted(
        set(all_configured_edges) - set(configured_edges)
    )
    ...
    return {
        ...
        "all_configured_edge_count": len(all_configured_edges),
        "observed_edge_count": sum(bool(groups.get(key)) for key in configured_edges),
        "evaluation_scope": evaluation_scope,
        "declared_route_edges": sorted(declared_route_edges),
        "ignored_configured_edges": ignored_configured_edges,
        "edges": details,
    }
