    ...
    for node_data in drift_nodes:
        ...
        for index, variation in enumerate(
            ...
            if not receiving_positions:
                ...
                continue
            if len(receiving_positions) != 1:
                issues.append(
                    {
                        "field": field,
                        "problem": (
                            "receiving node appears more than once after the "
                            "source in the schedule full path"
                        ),
                        "path": path,
            ...
            if stage_contract is None:
                issues.append(
                    {
                        "field": field,
                        "problem": (
                            "matching schedule path stage is missing from the "
                            "route contract"
                        ),
                        "path": path,
                        "stage": stage,
                ...
                continue
            legal_ranges = stage_contract["arrival_ranges"]
