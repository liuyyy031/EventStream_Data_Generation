"""Versioned STPP v12 prompts and self-consistent Judge response handling."""
...
def _enforce_judgment_invariants(
    approved: bool, judgment: Dict[str, Any]
) -> Tuple[bool, Dict[str, Any]]:
    """Make control-flow fields obey the contract even if the model does not."""
    output = dict(judgment)
    issues = output.get("issues", [])
    if not isinstance(issues, list):
        issues = []
    approved = approved is True and output.get("approved") is True
    if approved:
        output["approved"] = True
        output["error_source"] = None
        output["issues"] = []
        output["judge_contract_consistent"] = True
        return True, output

    output["approved"] = False
    if output.get("error_source") not in {"agent1", "agent2"}:
        output["error_source"] = "agent2"
    output["issues"] = issues
    output["judge_contract_consistent"] = bool(issues)
    return False, output


class TemporalContractJudgeNetworkSTPPGenerator(SafeJudgeNetworkSDEGenerator):
    """Use STPP-only prompts without overwriting the original prompt files."""
    ...
    def parse_scenario_to_structured_json(
        ...
        _base_module.SCENARIO_PARSING_PROMPT = STPP_V12_SCENARIO_PARSING_PROMPT
        try:
