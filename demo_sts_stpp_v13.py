#!/usr/bin/env python3
"""STPP v13 entry point with separated spatial support and v12 time contracts."""
...
from safe_judge_generator_v13 import SpatiallyGroundedJudgeNetworkSTPPGenerator
from stpp_adapter_v12 import canonicalise_temporal_contract_v12
...
def _agent1_feedback_v13(judgment: Dict[str, Any]) -> str:
    findings = []
    ...
    return (
        ...
        "Obey all v13 temporal and spatial contracts. Give every node one "
        "unique, well-separated coordinate in [0,1]. Return only the revised "
        "scenario.\n\n"
        + "\n".join(findings)
    )


def parse_scenario_with_judge_audit_v13(
    generator: SpatiallyGroundedJudgeNetworkSTPPGenerator,
    scenario: str,
    ...
    print("\n=== STPP v13 spatial/temporal Agent-Judge validation loop ===")
    while True:
        ...
        if (
            ...
            current_scenario = generator.generate_scenario_description(
                ...
                previous_feedback=_agent1_feedback_v13(last_judgment),
...
def _write_outputs_v13(
    ...
    prefix = f"{domain_clean}_node{ts_data.shape[0]}_stpp_v13_{timestamp}"
    complete_data = {
        "timestamp": timestamp,
        "generator_family": "spatially_grounded_temporal_contract_stpp_v13",
    ...
    with description_path.open("w", encoding="utf-8") as handle:
        handle.write("=== STReasoner Spatially Grounded STPP v13 ===\n\n")
        ...
        handle.write(
            ...
            f"{spatial['final_minimum_normalized_separation']}\n"
        )
        ...
        handle.write("Spatial layout audit\n--------------------\n")
...
def demo_stpp_generation_v13(
    enabled_judges: List[int] | None = None,
    enable_logging: bool = True,
    num_nodes: int = 3,
    domain: str = "traffic",
    generate_viz: bool = False,
    stpp_config: STPPV13Config | None = None,
    output_dir: str = "output_stpp_v13",
) -> Dict[str, Any]:
    enabled_judges = [1] if enabled_judges is None else enabled_judges
    config = stpp_config or STPPV13Config()
    if generate_viz:
        print("v13 does not generate the legacy SDE visualization.")
    ...
    if 1 in enabled_judges:
        scenario, parsed, judge_audit, agent2_audit = (
            parse_scenario_with_judge_audit_v13(generator, scenario)
        )
    else:
        raw = generator.parse_scenario_to_structured_json(scenario)
        parsed, agent2_audit = _canonicalise_agent2_json_v8(raw)
        judge_audit = {
            "required": False, "approved": None, "attempts": 0,
            "used_unapproved_parse": False, "agent1_revisions": 0,
            "agent2_attempts_per_scenario": [],
            "termination_reason": "judge_not_required", "last_judgment": None,
        }
    ...
    ts_data, events, generation_info = simulate_stpp_for_streasoner_v13(
    ...
    data_files = _write_outputs_v13(
        scenario, structured_scenario, graph_metadata, ts_data, events,
        generation_info, domain, Path(output_dir),
    )
    if logger:
...
def main() -> int:
    ...
    parser.add_argument("--output_dir", default="output_stpp_v13")
    parser.add_argument(
        "--aggregation",
