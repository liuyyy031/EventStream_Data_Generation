#!/usr/bin/env python3
"""Parallel batch driver for explicit temporal-contract STPP v12."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v12 import demo_stpp_generation_v12
    from stpp_adapter_v12 import STPPV12Config
    ...
    if not quality["passed"]:
        ...
        raise RuntimeError(f"v12 quality gates did not pass: {failed}")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v12.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v12"),
    )
