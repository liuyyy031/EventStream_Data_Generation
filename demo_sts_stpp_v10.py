#!/usr/bin/env python3
"""STPP v10 entry point with focused Judge retries and path support."""
...
def parse_scenario_with_judge_audit_v10(
    generator: SafeJudgeNetworkSDEGenerator,
    scenario: str,
    max_agent2_attempts: int = 6,
    max_agent1_revisions: int = 2,
) -> Tuple[str, Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    """Retry Agent 2 in place; revise Agent 1 only when Judge assigns it."""
    ...
    print("\n=== STPP v10 focused Agent/Judge validation loop ===")
    while True:
        ...
        for agent2_attempt in range(1, max_agent2_attempts + 1):
            ...
            if feedback_history:
                cumulative_feedback = (
                    "Resolve every still-applicable item in this Judge feedback "
                    "history while preserving the original scenario exactly:\n\n"
        ...
        if (
            ...
            current_scenario = generator.generate_scenario_description(
                ...
                previous_feedback=generator._format_feedback_for_agent1(  # noqa: SLF001
                    last_judgment
                ),
...
def _write_outputs_v10(
    ...
    prefix = f"{domain_clean}_node{ts_data.shape[0]}_stpp_v10_{timestamp}"
    complete_data = {
        "timestamp": timestamp,
        "generator_family": "path_aware_cycle_free_graph_stpp_v10",
    ...
    with description_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("=== STReasoner Path-Aware Cycle-Free STPP v10 ===\n\n")
        ...
        handle.write(
            "Path-support roots added: "
            f"{cycle_audit.get('path_support_roots_added', 0)}\n"
        )
        handle.write(
            "Path-support events added: "
            f"{cycle_audit.get('path_support_events_added', 0)}\n"
        )
        handle.write(
            "Quality checks: "
        ...
        handle.write("Propagation and path-support audit\n----------------------------------\n")
...
def demo_stpp_generation_v10(
    enabled_judges: List[int] | None = None,
    enable_logging: bool = True,
    num_nodes: int = 3,
    domain: str = "traffic",
    generate_viz: bool = False,
    stpp_config: STPPV10Config | None = None,
    output_dir: str = "output_stpp_v10",
) -> Dict[str, Any]:
    enabled_judges = [1] if enabled_judges is None else enabled_judges
    config = stpp_config or STPPV10Config()
    if generate_viz:
        print("v10 does not generate the legacy SDE visualization.")
    ...
    if 1 in enabled_judges:
        scenario, structured_scenario, judge_audit, canonical_audit = (
            parse_scenario_with_judge_audit_v10(generator, scenario)
        )
    else:
    ...
    ts_data, events, generation_info = simulate_stpp_for_streasoner_v10(
...
def main() -> int:
    ...
    parser.add_argument("--output_dir", default="output_stpp_v10")
    parser.add_argument(
        "--aggregation",
    ...
    result = demo_stpp_generation_v10(
    ...
    audit = info["propagation_cycle_audit"]
    ...
    print(f"Path-support roots added: {audit.get('path_support_roots_added', 0)}")
    print(f"Path-support events added: {audit.get('path_support_events_added', 0)}")
