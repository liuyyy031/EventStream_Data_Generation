#!/usr/bin/env python3
"""Parallel batch driver for quality-gated STPP v3."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v3 import demo_stpp_generation_v3
    from stpp_adapter_v3 import STPPV3Config
    ...
    if not result["generation_info"]["quality_report"]["passed"]:
        raise RuntimeError("v3 quality gates did not pass")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v3.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v3"),
    )
