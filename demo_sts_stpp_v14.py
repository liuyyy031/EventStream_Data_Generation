#!/usr/bin/env python3
"""STPP v14 entry point with node-local arrivals and schedule-aware causality."""
...
from safe_judge_generator_v14 import EventSemanticJudgeNetworkSTPPGenerator
from stpp_adapter_v13 import canonicalise_spatial_layout_v13
from stpp_adapter_v14 import (
    STPPV14Config,
    canonicalise_temporal_contract_v14,
    simulate_stpp_for_streasoner_v14,
)


def _agent1_feedback_v14(judgment: Dict[str, Any]) -> str:
    findings = []
    ...
    return (
        ...
        "Obey every v14 temporal, spatial, node-local arrival, schedule-ID, "
        "and demand-source propagation contract. Return only the revised "
        "scenario.\n\n"
        + "\n".join(findings)
    )


def parse_scenario_with_judge_audit_v14(
    generator: EventSemanticJudgeNetworkSTPPGenerator,
    scenario: str,
    ...
    print("\n=== STPP v14 node-local schedule-aware Agent/Judge loop ===")
    while True:
        ...
        if (
            ...
            current_scenario = generator.generate_scenario_description(
                ...
                previous_feedback=_agent1_feedback_v14(last_judgment),
...
def _write_outputs_v14(
    ...
    prefix = f"{domain_clean}_node{ts_data.shape[0]}_stpp_v14_{timestamp}"
    complete_data = {
        "timestamp": timestamp,
        "generator_family": "node_local_schedule_aware_stpp_v14",
    ...
    with description_path.open("w", encoding="utf-8") as handle:
        handle.write("=== STReasoner Node-Local Schedule-Aware STPP v14 ===\n\n")
        ...
        handle.write(f"Judge response consistent: {_judge_contract_consistent(judge)}\n")
        ...
        handle.write(
            "Demand sources without outgoing edges: "
            + json.dumps(audit["demand_sources_without_outgoing_edges"])
            + "\n"
        )
        handle.write(
            "Demand sources without event schedules: "
            + json.dumps(audit["demand_sources_without_event_schedules"])
            + "\n"
        )
        handle.write(
            "Propagated variation violations: "
            f"{len(audit['propagated_variation_violations'])}\n"
        )
        handle.write(
            "Schedule metadata violations: "
            f"{len(audit['schedule_metadata_violations'])}\n"
        )
        handle.write(
            "Causal window violations: "
            f"{len(audit['causal_window_violations'])}\n"
        )
        handle.write(
        ...
        handle.write("Event semantic audit\n--------------------\n")
...
def demo_stpp_generation_v14(
    enabled_judges: List[int] | None = None,
    enable_logging: bool = True,
    num_nodes: int = 3,
    domain: str = "traffic",
    generate_viz: bool = False,
    stpp_config: STPPV14Config | None = None,
    output_dir: str = "output_stpp_v14",
) -> Dict[str, Any]:
    enabled_judges = [1] if enabled_judges is None else enabled_judges
    config = stpp_config or STPPV14Config()
    if generate_viz:
        print("v14 does not generate the legacy SDE visualization.")
    ...
    if 1 in enabled_judges:
        scenario, parsed, judge_audit, agent2_audit = (
            parse_scenario_with_judge_audit_v14(generator, scenario)
        )
    else:
    ...
    generator.seq_len = seq_len
    structured_scenario, semantic_audit = canonicalise_temporal_contract_v14(
        parsed, seq_len
    )
    ...
    ts_data, events, generation_info = simulate_stpp_for_streasoner_v14(
    ...
    quality["checks"]["temporal_contract_valid"] = semantic_audit["passed"]
    quality["checks"]["temporal_contract_repair_free"] = semantic_audit[
        "repair_free"
    ]
    quality["checks"]["no_causal_window_violations"] = not semantic_audit[
        "causal_window_violations"
    ]
    quality["checks"]["all_demand_sources_have_outgoing_edges"] = not (
        semantic_audit["demand_sources_without_outgoing_edges"]
    )
    quality["checks"]["all_demand_sources_have_event_schedules"] = not (
        semantic_audit["demand_sources_without_event_schedules"]
    )
    quality["checks"]["propagated_variation_contract_valid"] = not (
        semantic_audit["propagated_variation_violations"]
    )
    quality["checks"]["schedule_metadata_valid"] = not semantic_audit[
        "schedule_metadata_violations"
    ]
    quality["checks"]["propagation_baselines_are_ambient"] = not semantic_audit[
    ...
    quality["temporal_contract_audit"] = semantic_audit
    quality["event_semantic_audit"] = semantic_audit
    ...
    generation_info["temporal_contract_audit"] = semantic_audit
    generation_info["event_semantic_audit"] = semantic_audit
...
def main() -> int:
    ...
    parser.add_argument("--output_dir", default="output_stpp_v14")
    parser.add_argument(
        "--aggregation",
