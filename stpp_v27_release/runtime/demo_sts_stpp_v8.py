#!/usr/bin/env python3
"""Integrated v8 entry point with explicit event peaks and canonical Judge JSON."""
...
def _canonicalise_agent2_json_v8(
    parsed_json: Dict[str, Any],
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    """Collapse accidental one-step ranges in propagated event timestamps."""
    canonical = copy.deepcopy(parsed_json)
    changes: List[Dict[str, Any]] = []
    drift = canonical.get("drift_patterns", {}) or {}
    for node_data in drift.get("nodes", []) or []:
        ...
        for index, variation in enumerate(
            ...
            if end != start + 1:
                continue
            variation["time"] = start
            changes.append(
                {
                    "field": (
                        f"drift_patterns.nodes[{node_id}]."
                        f"propagated_variations[{index}].time"
                    ),
                    "before": value,
                    "after": start,
                    "reason": "collapsed accidental one-step event-time range",
                }
            )
    return canonical, {
        "changes": changes,
...
def parse_scenario_with_judge_audit_v8(
    ...
    final_canonical_audit: Dict[str, Any] = {"changes": [], "num_changes": 0}
    attempts = 0
    last_judgment: Dict[str, Any] | None = None

    print("\n=== STPP v8 canonical Agent/Judge validation loop ===")
    for outer_iter in range(max_outer_iterations):
        ...
        for inner_iter in range(max_inner_iterations):
            ...
            parsed_json, final_canonical_audit = _canonicalise_agent2_json_v8(
                raw_parsed
            )
            ...
            previous_scenario = current_scenario
            agent1_feedback = (
                "Agent 2 repeatedly failed strict parsing. Rewrite with "
                "self_generated_peaks and propagated event times as explicit "
                "integer lists; distinguish exact events from time ranges; "
                "and label every pattern origin.\n\n"
                + feedback
            )
            needs_scenario_revision = True
            break
    ...
    print(
        "WARNING: Judge 1 did not approve; diagnostic output will fail quality."
    )
    return current_scenario, parsed_json, {
        "required": True,
        "approved": False,
...
def _write_outputs_v8(
    ...
    prefix = f"{domain_clean}_node{ts_data.shape[0]}_stpp_v8_{timestamp}"
    complete_data = {
        "timestamp": timestamp,
        "generator_family": "explicit_event_peak_graph_stpp_v8",
    ...
    with description_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("=== STReasoner Explicit Event-Peak STPP v8 ===\n\n")
        ...
        handle.write(f"Propagated events: {generation_info['num_propagated_events']}\n")
        handle.write(f"Scenario seed events: {generation_info['num_scenario_seed_events']}\n")
        ...
        handle.write(
            f"Propagation-node root events: {quality['propagation_node_root_event_count']}\n"
        )
        handle.write(
            f"Self-generated peaks: {quality['num_self_generated_peaks']}\n"
        )
        handle.write(
        ...
        handle.write("Self-generated peak coverage\n----------------------------\n")
        handle.write(
            json.dumps(
...
def demo_stpp_generation_v8(
    enabled_judges: List[int] | None = None,
    enable_logging: bool = True,
    num_nodes: int = 3,
    domain: str = "traffic",
    generate_viz: bool = False,
    stpp_config: STPPV8Config | None = None,
    output_dir: str = "output_stpp_v8",
) -> Dict[str, Any]:
    enabled_judges = [1] if enabled_judges is None else enabled_judges
    config = stpp_config or STPPV8Config()
    if generate_viz:
        print("v8 does not generate the legacy SDE visualization.")
    ...
    ts_data, events, generation_info = simulate_stpp_for_streasoner_v8(
...
def main() -> int:
    ...
    parser.add_argument("--output_dir", default="output_stpp_v8")
    parser.add_argument(
        "--aggregation",
    ...
    result = demo_stpp_generation_v8(
