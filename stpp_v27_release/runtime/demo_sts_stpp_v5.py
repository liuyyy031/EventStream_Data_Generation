#!/usr/bin/env python3
"""Non-destructive v5 entry point with semantic lag and envelope reporting."""
...
from demo_sts_stpp_v4 import repair_fractional_edge_lags
from stpp_adapter_v5 import STPPV5Config, simulate_stpp_for_streasoner_v5


def repair_semantic_edge_lags_v5(
    scenario: str,
    structured_scenario: Dict[str, Any],
    sampling_minutes: float,
    immediate_lag_epsilon_steps: float,
) -> List[Dict[str, Any]]:
    """Preserve explicit zero lag while still repairing lost fractional lags."""
    report = repair_fractional_edge_lags(
        scenario, structured_scenario, sampling_minutes
    )
    edges = structured_scenario.get("edges", [])
    for edge, item in zip(edges, report):
        text = str(item.get("matched_text", ""))
        explicit_zero = bool(
            re.search(
                r"time\s*_?lag\s*[:=]\s*0(?:\.0+)?\b", text, re.IGNORECASE
            )
            or re.search(r"\bimmediate(?:ly)?\b", text, re.IGNORECASE)
        )
        if item.get("source") != "zero_lag_fallback" or not explicit_zero:
            item["semantic_lag_steps"] = float(edge["time_lag"])
            continue
        edge["time_lag"] = float(immediate_lag_epsilon_steps)
        edge["time_lag_repair_source"] = "explicit_zero_lag"
        item.update(
            {
                "semantic_lag_steps": 0.0,
                "repaired_value_steps": float(immediate_lag_epsilon_steps),
                "source": "explicit_zero_lag",
                "note": (
                    "scenario semantics are immediate; epsilon is used only "
                    "to preserve strict parent-child time ordering"
                ),
            }
        )
    return report
...
def _write_outputs_v5(
    ...
    prefix = f"{domain_clean}_node{ts_data.shape[0]}_stpp_v5_{timestamp}"
    complete_data = {
        "timestamp": timestamp,
        "generator_family": "semantics_aware_inbound_envelope_graph_stpp_v5",
    ...
    with description_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("=== STReasoner Semantics-Aware Inbound-Envelope STPP v5 ===\n\n")
...
def demo_stpp_generation_v5(
    enabled_judges: List[int] | None = None,
    enable_logging: bool = True,
    num_nodes: int = 3,
    domain: str = "traffic",
    generate_viz: bool = False,
    stpp_config: STPPV5Config | None = None,
    output_dir: str = "output_stpp_v5",
) -> Dict[str, Any]:
    ...
    if generate_viz:
        print("v5 does not generate the legacy SDE visualization.")
    config = stpp_config or STPPV5Config()
    ...
    lag_repair = repair_semantic_edge_lags_v5(
        scenario,
        structured_scenario,
        sampling_minutes,
    ...
    ts_data, events, generation_info = simulate_stpp_for_streasoner_v5(
...
def main() -> int:
    ...
    parser.add_argument("--output_dir", default="output_stpp_v5")
    parser.add_argument(
        "--aggregation",
    ...
    result = demo_stpp_generation_v5(
