#!/usr/bin/env python3
"""Parallel batch driver for path-aware cycle-free STPP v10."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v10 import demo_stpp_generation_v10
    from stpp_adapter_v10 import STPPV10Config
    ...
    if not quality["passed"]:
        ...
        raise RuntimeError(f"v10 quality gates did not pass: {failed}")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v10.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v10"),
    )
