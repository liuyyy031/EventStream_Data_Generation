#!/usr/bin/env python3
"""Parallel batch driver for lag-aware quota-balanced STPP v4."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v4 import demo_stpp_generation_v4
    from stpp_adapter_v4 import STPPV4Config
    ...
    if not result["generation_info"]["quality_report"]["passed"]:
        raise RuntimeError("v4 quality gates did not pass")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v4.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v4"),
    )
