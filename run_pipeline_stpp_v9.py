#!/usr/bin/env python3
"""Parallel batch driver for cycle-free simple-path STPP v9."""
...
def _generate_one(
    ...
    from demo_sts_stpp_v9 import demo_stpp_generation_v9
    from stpp_adapter_v9 import STPPV9Config
    ...
    if not quality["passed"]:
        ...
        raise RuntimeError(f"v9 quality gates did not pass: {failed}")
    src_pkl = result["data_files"]["pickle"]
    dst_pkl = os.path.join(
        out_dir, f"task_{idx:04d}_{_slug(domain)}_n{num_nodes}_stpp_v9.pkl"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data_generation", "batch_output_stpp_v9"),
    )
